...
try{    
        ...
        $sql1 = "SELECT `views` FROM `posts` WHERE id = ?;";
        $stmt1 = $conn->prepare($sql1);
        $id= $_GET["id"];
        $stmt1->execute([$id]);
        $result1 = $stmt1->fetch();

       if(is_array($result1) || is_object($res)){
            foreach($result1 as $res){ 
                 $viewCount = ++$res;
                 $sql2      = "UPDATE `posts` SET `views`=$viewCount WHERE id = ?;";
...
<?php include_once("secondNavbar.php");?>
    ...
    <header class="page-header page-header-mini">
        <h1 class="title"><?php echo $result["postTitle"];?></h1>
        <ol class="breadcrumb pb-0">
            <li class="breadcrumb-item"><a href="index.php">Home</a></li>
            <li class="breadcrumb-item active" aria-current="page"><?php echo $result["postTitle"];?></li>
        </ol>
    </header>
    <!-- End Of Page Header -->
            ...
            <div class="page-content">
                <div class="card">
                    <div class="card-header pt-0">
                        <h3 class="card-title mb-4"><?php echo $result["postTitle"];?></h3>
                        <div class="blog-media mb-4">
                            <img src="assets/imgs/<?php echo $result["image"];?>" alt="<?php echo $result["image"];?>" class="w-100">
                            <a href="#" class="badge badge-primary">#Salupt</a> 
                        </div>  
                        <small class="small text-muted">
                            <a href="#" class="text-muted">BY Admin</a>
                            <span class="px-2">·</span>
                            <span><?php echo $result["date_created"];?></span>
                            <span class="px-2">·</span>

                            <input type = "hidden" name ="id" value="<?php echo $id;?>">
                            
                            <a href="#" class="text-muted"><?php echo "views: ".++$result1["views"];?></a>
                            <span class="px-2">·</span>
                            <a href="#" class="text-muted">36 Comments</a>
                        </small>
                    </div>
